...
void HowToPlay(){
	...
	if(how_to_play == NULL){
		printf("File Doesn't Exist"); // ketika file tidak ditemukan (tidak 1 folder)
	}else {
...
void drawGameboard() { // Author: ertrldtcu on Github, Link:  https://github.com/ertrldtcu/sos-game-in-c , modified by Raka Mahardika
	int i, j;
	printf("\n\n\n");
	// top line
	...
	for (i = 0; i <= dimensi; i++) {
		...
		else if (sos[i - 1][j - 1] == 'S' || sos[i - 1][j - 1] == 'O') printf(" %c  \xb3 ", sos[i - 1][j - 1]); //untuk menjaga bentuk papan agar tidak bergeser saat mengisi input
...
int controlSOS() { // Author: ertrldtcu on Github, Link:  https://github.com/ertrldtcu/sos-game-in-c
	int get;
	get = 0; 
	if (sos[sRow - 1][sCol - 1] == 'S') { // ngecek dari huruf s
		if (sos[sRow - 3][sCol - 3] == 'S' && sos[sRow - 2][sCol - 2] == 'O'){
		get++; }																		 // ngecek horizontal 
		if (sos[sRow - 3][sCol - 1] == 'S' && sos[sRow - 2][sCol - 1] == 'O'){
		get++; }																		 // ngecek vertikal keatas 
		...
		get++; }															; // ngecek diagonal kanan bawah 
	}
	else {
		if (sos[sRow - 2][sCol - 2] == 'S' && sos[sRow][sCol] == 'S') {
		get++; }															 // ngecek diagonal dari huruf O
		if (sos[sRow - 2][sCol - 1] == 'S' && sos[sRow][sCol - 1] == 'S'){
		get++; }															 // ngecek vertikal dari huruf O
		if (sos[sRow - 2][sCol] == 'S' && sos[sRow][sCol - 2] == 'S'){
		get++; }															 // ngecek diagonal dari huruf O
		if (sos[sRow - 1][sCol - 2] == 'S' && sos[sRow - 1][sCol] == 'S') {
		get++; }															 // ngecek horizontal dari huruf O
	}
...
void gamePlayPVP(){ // REFERENCE by ertrldtcu on Github, Link:  https://github.com/ertrldtcu/sos-game-in-c , modified and integrated by Syifa Khairina
    while (counter <= dimensi * dimensi){
		...
		int input = takeInput();
			if(input){
			int point = controlSOS(); // point+1 if true
			if (!point) {
...
int CekKotakKosong(int kotak){   					//valid position
...
void gamePlayPVC(){ // REFERENCE by ertrldtcu on Github, Link:  https://github.com/ertrldtcu/sos-game-in-c , modified and integrated by Gian Sandrova
	int i, j;
	while(counter<=dimensi*dimensi){
	system("cls");
	drawScoreboard();
...
void gamePlayPVCPro(){ // author: hadassah yanovsky on Github
	currentPlayer=0;
	drawScoreboardPVC();
	ResetBoard();
	while(!CekKotak()) {
